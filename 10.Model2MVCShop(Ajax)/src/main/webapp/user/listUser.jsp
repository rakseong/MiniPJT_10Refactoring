...
/* 			$("select[name='searchCondition'] option:contains('회원명')").on('click',function(){
				$("select[name='searchCondition'] option").each(function(index,elem){
					$(elem).remove;
				})
				$(this).append('<option value="0"  ${ ! empty search.searchCondition && search.searchCondition==0 ? "selected" : "" }>회원ID</option>')
				$(this).append('<option value="1" selected ${ ! empty search.searchCondition && search.searchCondition==1 ? "selected" : "" }>회원명</option>')			
			}) */
// 			 <td align="right">
...
// 			</td>
			 ...
			 $(".ct_input_g[name=searchKeyword]").on('keydown',function(){
